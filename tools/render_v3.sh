#!/bin/bash
# ROOTLINE intro v3 — smoother Richard take. Frame counts read live from
# seg_frames.json (written by timings_v3.py from the same take's alignment).
set -e
...
F1=$(python -c "import json;print(json.load(open('seg_frames.json'))['1'])")
F2=$(python -c "import json;print(json.load(open('seg_frames.json'))['2'])")
F3=$(python -c "import json;print(json.load(open('seg_frames.json'))['3'])")
F4=$(python -c "import json;print(json.load(open('seg_frames.json'))['4'])")
F5=$(python -c "import json;print(json.load(open('seg_frames.json'))['5'])")
...
echo "frames: $F1 $F2 $F3 $F4 $F5 $F6 $F7"
...
"$FF" -y -i "$IMG/hero.png" -vf "$(kb "$IMG/hero.png" $F1),vignette=PI/4.5,drawbox=x=0:y=0:w=iw:h=ih:color=black@0.35:t=fill,drawtext=$FB:textfile=title.txt:fontsize=170:fontcolor=$GOLD:x=(w-text_w)/2:y=h*0.38:shadowcolor=black@0.85:shadowx=4:shadowy=4,drawtext=$FR:textfile=title_sub.txt:fontsize=52:fontcolor=$CREAM:x=(w-text_w)/2:y=h*0.38+230:shadowcolor=black@0.8:shadowx=3:shadowy=3,format=yuv420p" -frames:v $F1 -c:v libx264 -preset veryfast -crf 18 seg/seg1.mp4 2>&1 | grep -E "error|Error" || true
echo "done seg1"

step thumb-culture.png    seg2.mp4 $F2 s1m.txt s1s.txt
step thumb-film.png       seg3.mp4 $F3 s2m.txt s2s.txt
step thumb-roots.png      seg4.mp4 $F4 s3m.txt s3s.txt
step thumb-blacktech.png  seg5.mp4 $F5 s4m.txt s4s.txt
